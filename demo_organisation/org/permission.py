...
class OrgCreation(BasePermission):
    def has_permission(self, request, view=None):
        ...
        if request.data.get("parent") is not None:
            org = CustomOrganization.objects.filter(
...
class IsOwnerOrAdmin(BasePermission):
    def has_permission(self, request, view=None):
        ...
        if org:
            ...
            if org_user and (
                OrganizationOwner.objects.filter(organization_user=org_user).exists()
                or org_user.is_admin
            ):
                return True
        return False
...
class IsOwner(BasePermission):
    def has_permission(self, request, view=None):
        ...
        if (
            org_user
            and OrganizationOwner.objects.filter(
                organization_user=org_user,
